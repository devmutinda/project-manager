export default function SideBar({ onSelect, projects, onViewProject, selectedIndex }) {
        ...
        <aside className="w-1/3 px-8 py-16 bg-stone-900 text-stone-50 md:w-72 rounded-r-xl">
            ...
            <ul className="mt-8 mb-4 text-stone-400">
                {projects.map((project, index) => {
                    ...
                    return (
                        <li>
                            <button className={cssClasses} key={index} onClick={() => handleClick(index)} >{project.title}</button>
                        </li>
                    );
                })}
            </ul>
